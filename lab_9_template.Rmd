...
# Relationship appears linear. There are a few outliers in Regular and Short Sale homes, but not enough reason to omit. 
...
# Forclosure selected as reference level. Arroyo Grande selected as reference level. 

# Makes no sense to include PricePerSqFt. PricePerSqFt is calculated from the dependent variable Price. Can result in biased overfitting of model results. 

# This model is bad. Bedrooms, SqFt, and PricePerSqFt all represent the same metric (size of the house). 
...
# Arroyo Grande is the reference variable. 

# If two houses are the same in terms of all other variables in the model (SqFt and Status), then would expect a house in San Luis Obispo to sell for $34,525.40 more than a house in Arroyo Grande. 

# For every one SqFt increase in house size, expect price to rise by $254.90 if all other variables are held constant. 

# If all other variables are held constant, expect StatusRegular house to sell for $209,998.90 more than Foreclosure house. 

# b. Model summary:

summary(homes_lm2)

# Use the adjusted R-squared value for multiple predictor variables. 
...
# Run homes_sub$Status in the console. Should say 'factor'
...
# Focus on the first two graphs. Look at the residual spread (heteroscedasticity) in graph 1 and normality (linearity) in graph 2. 

# Overall, heteroscedasticity is ok and residuals definitely look normal. Concepturally and mathematically models make sense. 

# 188 would be a good house to buy, model predicts the value of 188 to be higher than what it's priced as. 
...
# First model has a lower AIC value. But AIC predicts best model based on mathematics alone. 

# Answer: which would you pick? 

# Homes_lm3 is the best model in this case because: heteroscedasticity is ok and residuals definitely look normal. Concepturally and mathematically homes_lm3 make sense.
...
# Creates a sequence from 500-3500 SqFt for each city
# Gives 20 regular, 20 short sale, 20 foreclosure
...
# Fit.fit predicts price at which a home will sell for a give city, SqFt, and status. 
